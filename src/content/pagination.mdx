---
title: Pagination
description: Read long lists from the GitLite API with page and limit, and use the Link and X-Total-Count headers.
...
| `limit` | integer | `30` | Items per page. Values of `0` or less use the default of `30`. Values above `50` are reduced to `50`. |

The default (`30`) and the maximum (`50`) come from the server's
...
<Tip>
  ...
  at most `50` items, so do not assume a short page means you reached the end.
...
| `Access-Control-Expose-Headers` | `Link, X-Total-Count` | Lets browser clients read the two headers above. |
...
parameters receives 30 repositories and these headers:
...
<Tabs>
  ...
  <Tab title="JavaScript">
    ```js
    let url = 'http://localhost:3000/api/v1/user/repos?limit=50'
    const all = []
    while (url) {
