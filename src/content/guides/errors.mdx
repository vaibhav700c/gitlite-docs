...
| `Authorization: token <value>` where the token does not exist | `invalid username, password or token` |
...
<Tabs>
  ...
  <Tab title="JavaScript">
    ```js
    for (const headers of [{}, { Authorization: 'token 0000000000000000000000000000000000000000' }]) {
  ...
  <Tab title="JavaScript">
    ...
    async function gitlite(path, init = {}) {
      const res = await fetch(`http://localhost:3000/api/v1${path}`, {
        ...init,
        headers: { Authorization: `token ${process.env.GITLITE_TOKEN}`, ...init.headers },
      })
      if (!res.ok) {
